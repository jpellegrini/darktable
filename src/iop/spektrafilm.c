...
typedef struct dt_iop_spektrafilm_params_t
{
  ...
  float exposure_ev;        // $MIN: -12.0 $MAX: 12.0 $DEFAULT: 0.0 $DESCRIPTION: "film exposure"
  ...
  /* Whether the finished colour goes through the output gamut compressor. The
...
typedef struct dt_iop_spektrafilm_gui_data_t
{
  ...
  GtkWidget *section_page, *section_container;
  GList *sections; /* dt_gui_collapsible_section_t*, owned */
  /* List index of the "auto" entry in the paper combobox, whose label is
     rewritten to name the paper it resolves to. Recorded when the list is
     built: entries ahead of it would otherwise silently shift it. */
  int paper_auto_pos;
...
#pragma omp parallel for default(none) shared(plane, gbuf) firstprivate(npix)              \
    ...
    /* Upstream's grain blur (GrainParams.blur, params_schema.py) is a
       ...
       scaling lives entirely in particle DENSITY, which is pixel_um-driven
       above, and not in this smoothing pass. preview_scale is a SEPARATE,
       darktable-only
...
/* Separable Young-van Vliet pass on the device, the GPU half of
   ...
   Both paths must run this same recursion: a blur of a different shape produces
   a different halo width for the same sigma, and every spatial radius in the
   module was chosen against this one. See sf_gauss_yvv_coeffs. */
                          ...
                          dt_iop_module_t *self)
{
  ...
  /* scan-film follows the film's natural mode on a film switch: slides and
     ...
     print stage. The user can still choose freely afterwards -- this only
     re-baselines when the film itself changes, like the paper auto-follow.
     The paper combobox carries the state now, so move it with the value. */
  if(p->scan_film != e->positive)
  {
    p->scan_film = e->positive;
    DT_ENTER_GUI_UPDATE();
    dt_bauhaus_combobox_set_from_value(g->paper, p->scan_film ? -2 : -1);
    DT_LEAVE_GUI_UPDATE();
  }
  /* Unconditional: the paper combobox follows the film's own positive flag,
     which can change while scan_film does not -- selecting "none" on a
     negative and then switching to a positive stock leaves scan_film already
     set, and the combobox would stay live on a film that has no print
     stage. */
...
static void _update_paper_auto_entry(dt_iop_module_t *self)
{
  ...
  /* The auto entry is added before any section header, so its list index is
     fixed whatever paper groups the pack turns out to contain -- but it is not
     necessarily the first entry, so use the index recorded when the list was
     built rather than assuming one. */
  dt_bauhaus_combobox_set_entry_label(g->paper, g->paper_auto_pos, label);
                                       ...
                                       const sf_prof_entry_t *e)
{
  ...
  /* Never hidden. The print slider is the only widget in the print "chemistry"
     section, so hiding it leaves a collapsible with an empty body -- a heading
     and a disclosure arrow that open onto nothing, which reads as a broken
     panel rather than as an option that does not apply here. A stock
     characterised at one development still has a development time, and the
     slider shows it; it just cannot be moved, which greying it out says
     plainly. */
  ...
  if(have)
  {
    ...
    gtk_widget_set_tooltip_text(w,
                                _("development time. this stock is characterised at a "
                                  "single\n"
                                  "development, so there is nothing to choose.\n"
                                  "\n"
                                  "single-emulsion B&W stocks are the ones that carry a "
                                  "family:\n"
                                  "Double-X at 4/5/6.5/9/12 min, print film 2302 at "
                                  "2/3.5/5/7/9 min."));
}
                                            ...
                                            const dt_iop_spektrafilm_params_t *p)
{
  ...
  /* "auto" prints on a real paper while leaving paper_hash at 0 -- the link is
     the state, the destination is resolved from the film's target print. Looking
     up the hash alone found no paper there, so on the default selection the
     print slider sat dead at 0 min even when the paper it resolves to carries a
...
static void _update_print_sensitivity(dt_iop_module_t *self)
{
  ...
  /* The paper combobox is gated on the film, not on `printing`. A positive or
     reversal stock has no print stage at all, so a paper is not a choice there
     and picking one would put the image through a print it never has. For a
     negative the combobox stays live even while scanning, because it is the
     control that turns scanning off again -- gating it on `printing` would
     leave no way back. */
  {
    const sf_prof_entry_t *fe = _current_film_entry(g, p);
    gtk_widget_set_sensitive(g->paper, !(fe && fe->positive));
  }
...
/* --- generated presets: begin --- */

/* The introspection defaults, written out once so a preset below only has
   to name the parameters its look actually moves. A parameter added to
   dt_iop_spektrafilm_params_t therefore lands on its own default in every
   preset, instead of silently arriving as a zero.

   Keep this in step with the $DEFAULT annotations on the struct. */
/* The starting point every shipped preset is built from: a zeroed struct with
   each non-zero default written back. It has to list every field whose default
   is not zero, including new ones -- a field added to the struct and forgotten
   here ships in all 17 presets at zero, which for a boolean defaulting to TRUE
   means the preset silently turns the feature off. */
static void _preset_defaults(dt_iop_spektrafilm_params_t *p)
{
  ...
  p->halation_amount = 1.0f;
  p->halation_scale = 1.0f;
  p->boost_range = 0.3f;
  p->protect_ev = 4.0f;
  p->diffusion_strength = 0.5f;
  p->diffusion_scale = 1.0f;
  p->print_diffusion_strength = 0.5f;
  p->print_diffusion_scale = 1.0f;
  p->grain_on = TRUE;
  p->grain_amount = 1.0f;
  p->grain_blur = 1.0f;
  ...
  p->output_scale = 1.0f;
  p->grain_usm_sigma = 0.7f;
  p->grain_usm_amount = 1.5f;
  p->film_gamma_factor = 1.0f;
  p->film_gamma_factor_fast = 1.0f;
  p->film_gamma_factor_slow = 1.0f;
  p->scan_usm_sigma = 0.7f;
  p->scan_usm_amount = 0.7f;
...
{
  /* The scene-referred preset below exists only under one workflow, so the
     built-in presets have to be rebuilt when that setting changes. Without this
     flag _iop_preferences_changed() skips the module and the preset is created
     only if the workflow already happened to be spektrafilm when the module was
     first loaded -- selecting it later then does nothing until the library is
     rebuilt. filmicrgb, sigmoid and agx all set it for the same reason. */
  self->pref_based_presets = TRUE;

  dt_iop_spektrafilm_params_t p;

  /* The module is a view transform, so it takes the place of filmic, sigmoid or
     agx and not a place beside them. Selecting "scene-referred (spektrafilm)"
     auto-applies this preset and leaves those three unapplied, each of them
     already testing for its own workflow by name.

     Everything else the scene-referred workflow does still happens:
     dt_is_scene_referred() counts this value, so exposure and colour
     calibration are applied as usual. The film needs a correctly placed middle
     grey to land on its curve, so removing exposure would work against the
     module as much as against the workflow.

     Defaults, with no stock named: film_hash 0 resolves to the pack's first
     filming stock, which keeps the preset working whatever pack is installed.
     FOR_RAW | FOR_MATRIX matches the other view transforms -- a scene-referred
     starting point needs a camera the input profile can place. */
  ...
  /* black and white|crisp -- by Bazdesh, shared as "BW_Crispy TriX"

     Tri-X pushed a stop and a third onto a hard slow layer, scanner
     sharpening well up, acutance recovery off so the grain stays as drawn. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_trix");
  p.exposure_ev = 1.05f;
  ...
  p.diffusion_on = TRUE;
  p.diffusion_strength = 0.375f;
  p.grain_amount = 0.7f;
  p.grain_blur = 0.75f;
  p.grain_granularity = 0.7f;
  p.grain_uniformity = 0.9f;
  p.film_format_mm = 24.0f;
  p.output_luminance_boost = 1.3f;
  p.output_scale = 1.3f;
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 0.9f;
  p.film_gamma_factor_fast = 1.3f;
  p.film_gamma_factor_slow = 2.0f;
  p.push_pull_stops = 1.3f;
  p.scan_usm_sigma = 1.7f;
  p.scan_usm_amount = 1.0f;
  p.adaptation_bandwidth = FALSE;
  dt_gui_presets_add_generic(_("look|crisp black and white"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* black and white|soft -- by Bazdesh, shared as "BW_Low-cal TriX"

     The same stock pulled instead: flat fast and slow layers under a raised
     overall gamma, lifted on the output scale rather than in development. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_trix");
  p.exposure_ev = -0.7f;
  p.couplers_diffusion_um = 5.0f;
  ...
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 1.7f;
  p.film_gamma_factor_fast = 0.4f;
  p.film_gamma_factor_slow = 0.6f;
  p.push_pull_stops = -0.7f;
  p.scan_usm_sigma = 1.7f;
  p.scan_usm_amount = 1.0f;
  p.adaptation_bandwidth = FALSE;
  dt_gui_presets_add_generic(_("film|black and white"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* black and white|split-toned -- by Bazdesh, shared as "BW_Split-toned"

     Double-X printed onto colour paper, which is where the split tone comes
     from; the M/Y filtration sets which way each end of the scale leans. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_doublex");
  p.paper_hash = _name_hash("fujifilm_crystal_archive_typeii");
  p.exposure_ev = -0.7f;
  p.filter_m = 5.0f;
  p.filter_y = -3.0f;
  p.couplers_diffusion_um = 5.0f;
  p.couplers_tail_um = 30.0f;
  p.preflash_exposure = 0.03f;
  p.diffusion_on = TRUE;
  p.diffusion_strength = 0.25f;
  p.grain_blur = 0.75f;
  p.grain_granularity = 0.5f;
  p.output_scale = 1.2f;
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 1.7f;
  p.film_gamma_factor_fast = 0.5f;
  p.film_gamma_factor_slow = 0.7f;
  p.push_pull_stops = -1.0f;
  p.scan_usm_sigma = 1.7f;
  p.scan_usm_amount = 1.0f;
  p.glare_percent = 0.03f;
  p.development_min = 6.5f;
  p.adaptation_bandwidth = FALSE;
  dt_gui_presets_add_generic(_("look|split-toned"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* cinema|bleach bypass -- by Boris Hajdukovic, shared as "bleach bypass
     ryan"

     Silver retention emulated by driving same-layer inhibition to maximum
     and interlayer to zero: black-and-white contrast under a muted palette. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_vision3_200t");
  p.paper_hash = _name_hash("kodak_2393");
  p.exposure_ev = 1.39f;
  p.filter_m = 4.43f;
  p.filter_y = -0.15f;
  p.couplers_amount = 0.99f;
  p.couplers_inhibition_same = 3.0f;
  p.couplers_inhibition_inter = 0.0f;
  p.preflash_m_shift = -1.07f;
  p.preflash_y_shift = -0.76f;
  p.diffusion_on = TRUE;
  p.diffusion_filter_family = DT_SPEKTRAFILM_DIFF_CINEBLOOM;
  p.diffusion_strength = 0.183f;
  p.diffusion_scale = 4.0f;
  p.diffusion_warmth = 0.01f;
  p.film_format_mm = 24.89f;
  p.film_gamma_factor = 0.72f;
  p.film_gamma_factor_fast = 0.52f;
  p.film_gamma_factor_slow = 1.69f;
  p.push_pull_stops = 1.25f;
  p.scan_usm_sigma = 2.73f;
  dt_gui_presets_add_generic(_("look|bleach bypass"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* cinema|cool -- by Cristian, shared as "Kodak Vision3 250D"

     The same stock left on its own print target and steered cool-magenta
     through filtration and a strongly shifted preflash. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_vision3_250d");
  p.exposure_ev = 0.26f;
  p.filter_m = -3.75f;
  p.filter_y = -7.5f;
  p.couplers_inhibition_same = 1.93f;
  p.couplers_inhibition_inter = 0.5f;
  p.preflash_exposure = 0.02f;
  p.preflash_m_shift = -30.22f;
  p.preflash_y_shift = 35.74f;
  p.output_luminance_boost = 1.19f;
  dt_gui_presets_add_generic(_("film|cinema"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* cinema|Kino250D -- by Bazdesh, shared as "Kino250D"

     Vision3 250D on 2383 print stock, pulled and lifted hard on the pre-
     compression boost, with wide halation and a cool diffusion halo. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_vision3_250d");
  p.paper_hash = _name_hash("kodak_2383");
  p.exposure_ev = -0.7f;
  p.print_contrast = 1.2f;
  p.filter_m = -3.0f;
  p.filter_y = -4.5f;
  p.couplers_diffusion_um = 5.0f;
  p.couplers_tail_um = 30.0f;
  p.couplers_inhibition_same = 0.7f;
  p.couplers_inhibition_inter = 0.6f;
  p.preflash_exposure = 0.03f;
  p.preflash_m_shift = 5.0f;
  p.preflash_y_shift = 10.0f;
  p.halation_amount = 2.0f;
  p.halation_scale = 4.0f;
  p.diffusion_on = TRUE;
  p.diffusion_strength = 0.125f;
  p.diffusion_scale = 0.5f;
  p.diffusion_warmth = -1.5f;
  p.grain_amount = 0.5f;
  p.grain_granularity = 0.9f;
  p.output_luminance_boost = 1.9f;
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 0.9f;
  p.film_gamma_factor_fast = 0.7f;
  p.film_gamma_factor_slow = 1.1f;
  p.push_pull_stops = -0.85f;
  p.scan_usm_sigma = 1.4f;
  p.scan_usm_amount = 0.8f;
  dt_gui_presets_add_generic(_("look|kino 250D"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* everyday|classic -- by Cristian, shared as "Kodachrome 64"

     The gentler Kodachrome: no push, the separation carried by strong same-
     layer and weak interlayer inhibition instead of by contrast. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_kodachrome_64");
  p.couplers_diffusion_um = 23.6f;
  ...
  dt_gui_presets_add_generic(_("film|classic"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);
  /* everyday|contrast -- by Roberto Abella, shared as "C-200 [Contrast]
     [Best for: Color Calibrated - ETTL]"

     C200 on Endura Premier at a stop over, half-strength couplers and
     doubled grain under strong viewing glare. Expects a calibrated, right-
     biased exposure. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("fujifilm_c200");
  p.paper_hash = _name_hash("kodak_endura_premier");
  p.exposure_ev = 1.05f;
  p.couplers_amount = 0.5f;
  p.grain_amount = 2.0f;
  p.film_format_mm = 24.0f;
  p.film_gamma_factor = 0.9f;
  p.film_gamma_factor_fast = 0.8f;
  p.glare_percent = 0.75f;
  p.adaptation_surface = TRUE;
  dt_gui_presets_add_generic(_("film|contrast"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* everyday|neutral -- by Roberto Abella, shared as "X-Tra 400 [00] [Best
     for: Color Calibrated - well exposed]"

     X-Tra 400 on its own print target, half-strength couplers, lifted on
     both sides of the gamut compressor. Expects a calibrated, well exposed
     frame. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("fujifilm_xtra_400");
  p.couplers_amount = 0.5f;
  p.output_luminance_boost = 1.5f;
  p.output_scale = 1.2056f;
  p.glare_percent = 0.25f;
  p.adaptation_surface = TRUE;
  dt_gui_presets_add_generic(_("film|neutral"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* pastel|soft -- by Bazdesh, shared as "Pastel C200"

     C200 preflashed hard toward green, printed through a tight print-side
     diffusion filter, grain coarsened to sit on top of the flattened colour. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("fujifilm_c200");
  p.paper_hash = _name_hash("fujifilm_crystal_archive_typeii");
  p.exposure_ev = 1.0f;
  p.print_exposure_ev = -0.2f;
  p.filter_m = 5.0f;
  p.filter_y = -1.0f;
  p.couplers_amount = 0.75f;
  p.couplers_diffusion_um = 5.0f;
  p.couplers_tail_um = 15.0f;
  p.couplers_inhibition_inter = 0.9f;
  p.preflash_exposure = 0.05f;
  p.preflash_m_shift = -23.5f;
  p.preflash_y_shift = -30.0f;
  p.halation_scale = 3.0f;
  p.print_diffusion_on = TRUE;
  p.print_diffusion_strength = 0.75f;
  p.print_diffusion_scale = 0.5f;
  p.grain_amount = 1.3f;
  p.grain_granularity = 1.7f;
  p.output_luminance_boost = 1.25f;
  p.output_scale = 1.05f;
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 0.9f;
  p.film_gamma_factor_fast = 0.9f;
  p.film_gamma_factor_slow = 1.7f;
  p.push_pull_stops = 0.8f;
  p.scan_usm_sigma = 1.2f;
  p.scan_usm_amount = 1.0f;
  dt_gui_presets_add_generic(_("film|soft pastel"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* pastel|warm -- by Cristian, shared as "Kodak Portra 800"

     Portra 800 pulled slightly, warmed through magenta filtration and an
     offsetting preflash, then lifted on the pre-compression boost. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_portra_800");
  p.exposure_ev = 0.53f;
  p.print_exposure_ev = 0.13f;
  p.filter_m = 10.37f;
  p.filter_y = -2.87f;
  p.couplers_inhibition_same = 1.51f;
  p.couplers_inhibition_inter = 0.81f;
  p.preflash_exposure = 0.04f;
  p.preflash_m_shift = -28.46f;
  p.preflash_y_shift = -33.97f;
  p.output_luminance_boost = 1.64f;
  p.film_gamma_factor_slow = 1.32f;
  p.push_pull_stops = -0.32f;
  dt_gui_presets_add_generic(_("film|warm"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* portrait|bright -- by Roberto Abella, shared as "Kodak Portra 400
     [Bright - Endura Premier - No Glare] [Best for: slightly ETTL]"

     Portra 400 on Endura Premier, exposed well up with soft fast and slow
     layers and half the developer exhausted. Expects a slightly right-biased
     exposure. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_portra_400");
  p.paper_hash = _name_hash("kodak_endura_premier");
  p.exposure_ev = 1.4f;
  p.couplers_amount = 0.3f;
  p.output_luminance_boost = 1.3f;
  p.output_scale = 1.3f;
  p.film_gamma_factor_fast = 0.64f;
  p.film_gamma_factor_slow = 0.44f;
  p.film_developer_exhaustion = 0.5f;
  p.adaptation_surface = TRUE;
  dt_gui_presets_add_generic(_("film|portrait"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* portrait|contrast -- by Roberto Abella, shared as "Kodak Portra 400
     [Contrast - Endura Premier - Glare] [Best for: Color Calibrated - well
     exposed]"

     The same pairing left at box exposure, its contrast coming from
     exhausted development and a heavy viewing-glare simulation. Expects a
     calibrated, well exposed frame. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_portra_400");
  p.paper_hash = _name_hash("kodak_endura_premier");
  p.couplers_amount = 0.25f;
  p.film_developer_exhaustion = 0.45f;
  p.glare_percent = 0.82f;
  dt_gui_presets_add_generic(_("film|portrait contrast"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* creative|Velveetia -- by Bazdesh, shared as "Velveetia"

     Velvia 100 scanned direct, with the interlayer inhibition pulled back so
     the saturation the stock is known for survives the push. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("fujifilm_velvia_100");
  p.exposure_ev = 0.9f;
  p.couplers_diffusion_um = 5.0f;
  p.couplers_tail_um = 15.0f;
  ...
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 0.8f;
  p.film_gamma_factor_fast = 1.1f;
  p.film_gamma_factor_slow = 2.0f;
  p.push_pull_stops = 0.7f;
  p.scan_usm_sigma = 1.5f;
  p.scan_usm_amount = 1.0f;
  p.adaptation_bandwidth = FALSE;
  dt_gui_presets_add_generic(_("look|vivid"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* creative|Skodachrome -- by Bazdesh, shared as "Skodachrome"

     Kodachrome 64 scanned rather than printed, pushed and steepened in the
     slow layer: the punchiest of the three reversal looks. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_kodachrome_64");
  p.exposure_ev = 1.1f;
  p.couplers_amount = 0.6f;
  p.couplers_diffusion_um = 5.0f;
  p.couplers_tail_um = 15.0f;
  p.couplers_inhibition_inter = 0.6f;
  p.scan_film = TRUE;
  ...
  p.scan_usm_amount = 1.0f;
  p.adaptation_bandwidth = FALSE;
  dt_gui_presets_add_generic(_("look|skodachrome"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* landscape|foliage -- by Bazdesh, shared as "Gold200_foliage"

     Gold 200 on Endura Premier with the print contrast up and heavy yellow
     filtration, aimed at green subjects. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_gold_200");
  p.paper_hash = _name_hash("kodak_endura_premier");
  p.exposure_ev = -0.2f;
  p.print_contrast = 1.3f;
  p.filter_m = 4.5f;
  p.filter_y = 9.5f;
  p.couplers_amount = 0.85f;
  p.couplers_diffusion_um = 5.0f;
  p.couplers_tail_um = 20.0f;
  p.preflash_exposure = 0.03f;
  p.diffusion_strength = 0.125f;
  p.diffusion_scale = 0.5f;
  p.diffusion_warmth = -1.5f;
  p.grain_granularity = 1.1f;
  p.grain_usm_sigma = 0.0f;
  p.grain_usm_amount = 0.0f;
  p.film_gamma_factor = 0.9f;
  p.film_gamma_factor_fast = 0.9f;
  p.film_gamma_factor_slow = 1.1f;
  p.push_pull_stops = 0.45f;
  p.scan_usm_sigma = 1.2f;
  p.adaptation_bandwidth = FALSE;
  dt_gui_presets_add_generic(_("look|foliage"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);

  /* landscape|golden hour -- by Bazdesh, shared as "KinoGolden"

     Gold 200 on Ultra Endura, pushed half a stop into warm filtration, with
     the wide halation and cool diffusion halo of the cinema pair. */
  _preset_defaults(&p);
  p.film_hash = _name_hash("kodak_gold_200");
  p.paper_hash = _name_hash("kodak_ultra_endura");
  p.exposure_ev = 0.45f;
  p.filter_m = 4.0f;
  p.filter_y = -5.0f;
  p.couplers_diffusion_um = 5.0f;
  ...
  p.film_gamma_factor_fast = 0.9f;
  p.film_gamma_factor_slow = 1.1f;
  p.push_pull_stops = 0.5f;
  p.scan_usm_sigma = 1.2f;
  dt_gui_presets_add_generic(_("look|glow"), self->op,
                             self->version(), &p, sizeof(p), TRUE,
                             DEVELOP_BLEND_CS_RGB_SCENE);
}
...
void gui_update(dt_iop_module_t *self)
{
  ...
  /* No paper is a paper choice: the pipeline already treats "no print stage"
     and "scan the film" as the same state, and the two controls were coupled
     anyway -- the paper combobox was insensitive whenever scanning was on.
     Data -2 keeps it clear of -1 (auto) and of the list positions below. */
  dt_bauhaus_combobox_add_full(g->paper, _("none (scan the film)"),
                               DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT, GINT_TO_POINTER(-2), NULL, TRUE);
  /* ask the list where auto is about to land rather than assuming a position */
  g->paper_auto_pos = dt_bauhaus_combobox_length(g->paper);
  ...
  if(p->scan_film) ppos = -2;
  if(ppos >= -2) dt_bauhaus_combobox_set_from_value(g->paper, ppos);
...
/* Section heading with its own reset button.
   ...
   button resets exactly the widgets its own section holds.

   Each section owns a container and every widget in it belongs to that section,
   so the button carries the container and resets its children. */
static void _section_reset_clicked(GtkButton *button,
                                   dt_iop_module_t *self)
{
  if(darktable.gui->reset) return;
  GtkWidget *box = g_object_get_data(G_OBJECT(button), "sf_section_container");
  if(!box) return;
...
/* Open a collapsible section on the current page and point self->widget at its
   container, so every widget built afterwards lands inside it until the next
   call. The reset button sits in the section header and carries the container,
   so it resets exactly the widgets the section holds.

   `confname` persists the expanded state and must be unique across the module:
   several tabs use the same section label. Passing NULL builds a fixed section
   instead -- same container and same reset button, but always open and with no
   disclosure arrow -- for a section that should not be hidden behind a click.

   Both forms put their widgets in a container of their own, so the reset button
   works identically for either. Returns the section's root widget: the
   expander, or the wrapper holding the fixed form's heading and container. */
                               ...
                               const char *confname)
{
  ...
  /* self->widget is the page only on the first section of a page; afterwards it
     is the previous section's container. */
  if(self->widget != g->section_container) g->section_page = self->widget;
  GtkWidget *page = g->section_page;

  if(!confname)
  {
    /* header and container live in a wrapper, so the section is a single widget
       from the outside and can be hidden in one call */
    GtkWidget *hdr = dt_gui_hbox(dt_gui_expand(dt_ui_section_label_new(label)));

    GtkWidget *box = gtk_box_new(GTK_ORIENTATION_VERTICAL, DT_BAUHAUS_SPACE);

    GtkWidget *rst = dtgtk_button_new(dtgtk_cairo_paint_reset, 0, NULL);
    gtk_widget_set_tooltip_text(rst, _("reset only this section"));
    g_object_set_data(G_OBJECT(rst), "sf_section_container", box);
    /* No expand on the button: the label beside it already takes the spare
       width, so a plain add leaves the button at the right-hand end. Expanding
       it as well makes the box split the spare width between the two, which
       moves the button in from the edge. */
    dt_gui_box_add(hdr, rst);
    g_signal_connect(G_OBJECT(rst), "clicked", G_CALLBACK(_section_reset_clicked), self);

    GtkWidget *wrap = dt_gui_vbox(hdr, box);
    dt_gui_box_add(page, wrap);

    g->section_container = box;
    self->widget = box;
    return wrap;
  }

  dt_gui_collapsible_section_t *cs = g_malloc0(sizeof(*cs));
  dt_gui_new_collapsible_section(cs, confname, label, GTK_BOX(page), DT_ACTION(self));

  /* dt_gui_new_collapsible_section appends at the end of the box, which suits a
     module carrying a single trailing section but reverses the order of
     several. Move it to the front so sections appear in the order they are
     declared. */
  g_object_ref(cs->expander);
  gtk_container_remove(GTK_CONTAINER(page), cs->expander);
  dt_gui_box_add(page, cs->expander);
  g_object_unref(cs->expander);
  ...
  g_object_set_data(G_OBJECT(btn), "sf_section_container", cs->container);
  /* Same as above, and here the arrow is the thing that moves:
     dt_gui_new_collapsible_section() packs its label to expand and its toggle
     not to, so an expanding button added after them takes half the spare width
     and pushes the arrow off the edge. */
  dt_gui_box_add(hdr, btn);
  g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(_section_reset_clicked), self);

  g->sections = g_list_prepend(g->sections, cs);
  g->section_container = GTK_WIDGET(cs->container);
...
void gui_init(dt_iop_module_t *self)
{
  ...
  /* Children of these vertical boxes are added plain: a fixed-height bauhaus
     widget has no use for vertical growth, and across the column it already
     fills. dt_gui_expand() would ask for horizontal growth, which is not what
     a column wants. */
  ...
  dt_gui_box_add(g->data_box, g->data_status);
  ...
  dt_gui_box_add(g->data_box, g->data_button);

  gtk_widget_show(g->data_status);
  gtk_widget_show(g->data_button);
  dt_gui_box_add(sf_main_box, g->data_box);
  ...
  dt_gui_box_add(header_box, g->film);

  g->paper = dt_bauhaus_combobox_new(self);
  gtk_widget_set_tooltip_text(
      g->paper, _("print paper. \"none\" views the developed film directly, with no\n"
                  "print stage, which is the natural mode for positive and reversal\n"
                  "stocks; \"auto\" uses the paper named in the film stock's profile."));
  ...
  dt_gui_box_add(header_box, g->paper);
  ...
  gtk_widget_set_tooltip_text(g->film_format_combo,
                              _("common film/sensor gate presets; picking one sets the "
                                "frame long\n"
                                "edge slider below (pick \"custom\" to dial in an exact "
                                "value). the\n"
                                "preset names a film gauge (35mm) while the slider is the "
                                "frame's\n"
                                "long edge (36mm). both describe the same format."));
  _populate_format_combo(self);
  g_signal_connect(G_OBJECT(g->film_format_combo), "value-changed",
                   G_CALLBACK(_format_changed), self);
  dt_gui_box_add(header_box, g->film_format_combo);
  ...
  gtk_widget_set_tooltip_text(g->film_format_mm_slider,
                              _("physical frame size, long edge. sets the scale that "
                                "grain, scatter,\n"
                                "halation and diffusion are all computed at, so a smaller "
                                "format\n"
                                "shows every one of them proportionally larger for the same\n"
                                "print size."));
  ...
  _section_add(self, C_("section", "exposure"), NULL);

  g->exposure_ev = dt_bauhaus_slider_from_params(self, "exposure_ev");
  /* Dragging covers +-4 EV, the range this is used over. The hard range reaches
     +-12 for a typed value: an evening frame whose highlights were held by a
     large positive exposure upstream needs a comparable negative here to put
     the scene back on the film's curve, and +-4 cannot undo +8. */
  dt_bauhaus_slider_set_soft_range(g->exposure_ev, -4.0f, 4.0f);
  dt_bauhaus_slider_set_format(g->exposure_ev, _(" EV"));
  gtk_widget_set_tooltip_text(
      g->exposure_ev, _("film exposure compensation; with auto print exposure enabled, "
                        "print\n"
                        "exposure follows automatically so this has no net brightness "
                        "effect\n"
  ...
  gtk_widget_set_tooltip_text(
      g->push_pull_stops,
      _("push (positive) or pull (negative) processing: shoot at an effective\n"
        "ISO different from box speed, then under- or over-develop to\n"
        "compensate. combines an exposure shift with a derived contrast\n"
        "increase/decrease (approximate: the exact relationship depends on\n"
        "the specific film/developer combination, which isn't modeled here).\n"
        "Stacks with the granular gamma controls below for\n"
        "further fine-tuning."));

  _section_add(self, C_("section", "chemistry"), "plugins/darkroom/spektrafilm/expand_film_chemistry");
  ...
  gtk_widget_set_tooltip_text(
      g->film_gamma_factor,
      _("overall development contrast (morphs the film's density curves).\n"
        "extended or reduced development time, as in push/pull processing;\n"
        "1.0 = normal development."));
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("contrast of the fastest (most light-sensitive) emulsion sub-layer\n"
        "only, independent of the slow layer, since push/pull processing\n"
        "doesn't always affect every sub-layer equally."));
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("local developer depletion in dense (highly-exposed) areas: blends\n"
        "the highlight shoulder toward a self-limiting rolloff without\n"
        "shifting midgray (0 = off)."));
  ...
  _section_add(self, C_("section", "DIR couplers"), "plugins/darkroom/spektrafilm/expand_film_dir_couplers");
  ...
  gtk_widget_set_tooltip_text(g->couplers_inhibition_same,
                              _("how strongly a layer inhibits its own development. "
                                "raising it\n"
                                "flattens contrast within each channel."));
  ...
  gtk_widget_set_tooltip_text(g->couplers_inhibition_inter,
                              _("how strongly each layer inhibits the other two. this is "
                                "the part\n"
                                "that produces the saturation couplers are known for; at 0 "
                                "the\n"
                                "channels develop independently."));
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("how far the released inhibitor spreads through the emulsion.\n"
        "\n"
        "this is what turns the couplers into an edge effect: a wide spread\n"
        "lets a bright area hold back development well beyond its own edge,\n"
        "which reads as added clarity or sharpening. shorten it for a softer,\n"
        "more purely tonal coupler response; 5-20 um is the useful range.\n"
        "\n"
  ...
  gtk_widget_set_tooltip_text(
      g->couplers_tail_um,
      _("length of the long, faint tail on the spread above, which carries\n"
        "the inhibitor much further at low weight.\n"
        "\n"
        "it is what gives large bright areas a broad, gentle falloff. set the\n"
        "weight to 0 to drop it entirely."));
  ...
  gtk_widget_set_tooltip_text(g->couplers_tail_weight,
                              _("the share of the inhibitor that travels in the long tail. 0 "
                                "removes the tail."));

  /* The diffusion filter sits in front of the taking lens, so it belongs with
     the rest of the film stage rather than on a tab of its own. */
  _section_add(self, C_("section", "diffusion"),
               "plugins/darkroom/spektrafilm/expand_film_diffusion");

  /* Inside the section, not ahead of it: widgets built before _section_add()
     go into whichever container is still open, so on a page of collapsible
     sections a master toggle placed first lands in the previous section.
     Exempt, so resetting this section leaves the switch alone. */
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("diffusion filter type: black pro-mist (concentrated, punchy halo,\n"
        "deep blacks) / glimmerglass (tight, subtle, sharp-preserving) /\n"
        "pro-mist (broader, pastel, atmospheric) / cinebloom (frame-wide,\n"
        "slow-decaying veil)."));
  ...
  gtk_widget_set_tooltip_text(
      g->diffusion_scale,
      _("scales the radius of the diffusion halo. the amount of light is\n"
        "unchanged; it is spread further from each highlight. use diffusion\n"
        "strength to change the amount."));

  g->diffusion_warmth = dt_bauhaus_slider_from_params(self, "diffusion_warmth");
  gtk_widget_set_tooltip_text(g->diffusion_warmth,
                              _("diffusion halo warmth: >0 warm outer halo, <0 cool (added "
                                "on top of\n"
                                "the selected filter's own warmth bias)."));
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("first half of the film's sensitivity adaptation: a spectral bandpass\n"
        ...
        "ends of each channel while preserving white balance. on by default,\n"
        "and best left on: it is part of how the stock is characterised. no\n"
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("automatically compensate print exposure for film exposure changes,\n"
        "as a real printer would print to a fixed density; disable for film\n"
        "exposure to affect brightness directly, same as a fixed enlarger\n"
        "exposure time."));
  ...
  _section_add(self, C_("section", "chemistry"), "plugins/darkroom/spektrafilm/expand_print_chemistry");
  ...
  _section_add(self, C_("section", "preflash"), "plugins/darkroom/spektrafilm/expand_print_preflash");
  ...
  gtk_widget_set_tooltip_text(
      g->preflash_exposure,
      _("preflash exposure: a brief, uniform pre-exposure of the print\n"
        "through the film's base density, before the main print exposure.\n"
        "lifts shadows and reduces contrast (0 = off)."));
  ...
  gtk_widget_set_tooltip_text(g->preflash_m_shift,
                              _("magenta filtration for the preflash exposure only, Kodak "
                                "CC units\n"
                                "from neutral, independent of the main enlarger filtration "
                                "above."));
  ...
  gtk_widget_set_tooltip_text(g->preflash_y_shift,
                              _("yellow filtration for the preflash exposure only, Kodak "
                                "CC units\n"
                                "from neutral, independent of the main enlarger filtration "
                                "above."));

  /* the enlarger-side diffusion filter, alongside the rest of the print stage */
  _section_add(self, C_("section", "diffusion"),
               "plugins/darkroom/spektrafilm/expand_print_diffusion");

  /* inside the section, for the reason given on the film tab */
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("how much light is diverted into the print diffusion halo (0 = off).\n"
        ...
        "then 1 and 2. scrolling and the arrow keys move an eighth at a time.\n"
        "\n"
        "acts at the enlarger, so it blooms the printed image."));

  g->print_diffusion_scale = dt_bauhaus_slider_from_params(self, "print_diffusion_scale");
  gtk_widget_set_tooltip_text(
      g->print_diffusion_scale,
      _("scales the radius of the print diffusion halo. the amount of light\n"
        "is unchanged; it is spread further from each highlight. use print\n"
        "diffusion strength to change the amount."));
  ...
  gtk_widget_set_tooltip_text(g->print_diffusion_warmth,
                              _("print diffusion halo warmth: >0 warm outer halo, <0 cool "
                                "(added on\n"
                                "top of the selected filter's own warmth bias)."));
  ...
  _section_add(self, C_("section", "emulsion"), NULL);
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("how coarse the film's crystals are, relative to this stock's own\n"
        ...
        "\n"
        "this is the physical size control: the particles grow, there are\n"
        "fewer of them, and the grain becomes stronger as well as coarser,\n"
        "the same way a faster film differs from a slower one. it rebuilds\n"
        "the emulsion, so unlike grain strength it behaves the same on\n"
  ...
  gtk_widget_set_tooltip_text(
      g->grain_amount,
      _("how far the grain is allowed to move each pixel. it scales the drawn\n"
        "result; the emulsion is unchanged. 1.0 is film-accurate, 0 is off.\n"
        "\n"
        ...
        "same value reads much weaker on slide film. granularity above is the\n"
        "control for coarser grain."));
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("blur applied to the grain after it is drawn, in pixels. this softens\n"
        "the grain. the crystals themselves are unchanged.\n"
        "\n"
  ...
  gtk_widget_set_tooltip_text(
      g->grain_dye_cloud,
      _("each developed crystal leaves a small cloud of dye. this scales how\n"
        "far that cloud spreads, measured on the film itself, so it only\n"
        "becomes visible at magnifications where a single crystal covers more\n"
        "than a pixel."));
  ...
  gtk_widget_set_tooltip_text(
      g->grain_usm_sigma,
      _("radius of the recovery sharpening, in pixels. 0 switches it off. a\n"
        "broad radius puts the crispness back in the mid detail range, which\n"
        "keeps the sampling grid from showing through."));
  ...
  _section_add(self, C_("section", "scatter"), NULL);
  ...
  gtk_widget_set_tooltip_text(g->scatter_amount,
                              _("fraction of light that scatters inside the emulsion, "
                                "before the\n"
                                "halation bounce. 1.0 is film-accurate and is also the "
                                "maximum. it\n"
                                "means all of it, so nothing of the unscattered image "
                                "remains.\n"
                                "\n"
                                "this is why the whole frame softens: the scatter radius "
                                "is small (a\n"
                                "few um on film) but it applies everywhere. lower this if "
                                "you want a\n"
  ...
  gtk_widget_set_tooltip_text(g->scatter_scale,
                              _("scales the in-emulsion scatter radius. 1.0 is "
                                "film-accurate: the\n"
                                "radius the film model itself works at, and not normally "
                                "something\n"
                                "to change.\n"
                                "\n"
                                "above 1.0 you are past what the film model claims, and "
                                "the whole\n"
                                "frame softens quickly: the radius scales with the value, "
                                "so 4.0 is a\n"
                                "four times wider blur everywhere."));

  _section_add(self, C_("section", "halation"), NULL);
  ...
  gtk_widget_set_tooltip_text(g->halation_amount,
                              _("halation strength. 1.0 is film-accurate."));
  ...
  gtk_widget_set_tooltip_text(g->protect_ev,
                              _("protect tones below this many stops over mid-grey from the "
                                "boost"));
  ...
  gtk_widget_set_tooltip_text(
      ...
      _("scales the finished picture, after the gamut compressor.\n"
        ...
        "\n"
        "nothing rolls off behind the compressor, so this can push values\n"
        "past white for the rest of the pipeline to handle."));
  dt_color_picker_new(self, DT_COLOR_PICKER_AREA, g->output_scale);
  dt_bauhaus_widget_set_quad_tooltip(g->output_scale,
                                     _("pick brightest tone in the selected area and scale "
                                       "it to just below white."));

  _section_add(self, C_("section", "sharpness"), NULL);
  ...
  gtk_widget_set_tooltip_text(g->scan_usm_amount,
                              _("scanner sharpening strength (0 = off). 0.7 is what a scan "
                                "of the\n"
                                "film normally gets; leave at 0 if you prefer to sharpen "
                                "downstream."));

  _section_add(self, C_("section", "glare"), NULL);
...
void gui_cleanup(dt_iop_module_t *self)
{
  ...
  if(g)
  {
    ...
    {
      dt_gui_collapsible_section_t *cs = l->data;
      g_free(cs->confname);
      g_free(cs);
    }
    g_list_free(g->sections);
    g->sections = NULL;
    g_list_free_full(g->entries, g_free);
    g->entries = NULL;
  }
